<template>
    <el-main class="main">
      <el-main v-for="(friend,index) in circlefriendZanList " :key="index">
        <!--头像和名字-->
        <el-main class="first-auto">
            ...
            <el-main class="name">
              ...
              <span> <el-button type="warning "  size="mini" @click="followa(friend.message)">关注</el-button></span>
            </el-main>
  ...
  import axios from 'axios'
    export default {
      ...
      data(){
        return{
          sizeList:[],
          circleUrl:'https://ss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=522462713,1298435548&fm=11&gp=0.jpg',
          circlefriendZanList:[],
          remarkList:[],
          remark:{
            commentDesc:'',
      ...
      methods:{
        followa:function(user){
          this.follow.uid = this.user.id
          console.log(this.follow.uid)
          this.follow.followUser = user
          console.log(this.follow.followUser)
        ...
        findAllByZan:function(){
          axios.get("http://localhost:7000/s-circle of friend/circleFriend/circleFriendFindAllByNewByZanNums").then(res=>{
            console.log(11)
            if (res.data.code==200){
              console.log(res.data.data)
              this.circlefriendZanList = res.data.data
            }
          })
        },
      },
